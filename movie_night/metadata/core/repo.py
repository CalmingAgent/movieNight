...
from movie_night.metadata.movie_night_db import execute, executemany, commit, connection
...
class MovieRepo:
   ...
   #Google trend helps
    ...
    @staticmethod
    def population_share_by_country() -> Dict[str, float]:
        """Return a dictionary mapping ISO2 country codes to their share of movies in the database."""
        total = MovieRepo.total_movie_count()
        if not total:
            return {}
        rows = execute(
        ...
        return {r["origin"]: r["cnt"] / total for r in rows}

    @staticmethod
    def internet_penetration_by_country() -> Dict[str, float]:
        """Return a mapping of ISO2 country codes to estimated internet penetration ratios (0.0 to 1.0)."""
        # Return sensible defaults for common origins
        return {
            "US": 0.92,
            "GB": 0.95,
            "CA": 0.93,
